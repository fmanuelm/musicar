...
export class DataTableComponent implements OnInit, AfterViewInit {
  ...
  private clients: any;
  private arrClientDataTable: any[] = [['Airi Satou', 'Andrew Mike', 'Develop', '2013', '99,225', ''], ['Brielle Williamson', 'Mike Monday', 'Marketing', '2013', '49,990', 'btn-round']];
  ...
  async ngOnInit() {
    ...
    this.dataTable = {
      headerRow: ['Razón social', 'Razón comercial', 'NIT', 'Tipo cliente', 'País', 'Actions'],
      footerRow: ['Razón social', 'Razón comercial', 'NIT', 'Tipo cliente', 'País', 'Actions'],
      dataRows: this.arrClientDataTable,
      // dataRows: [
    ...
    await this.clientService.getClients().subscribe(resp => {
      const respuest = resp[0];
      // console.log('fuera: ', respuest);

      respuest.forEach(client => {
        // console.log('desde el fro', client);
        this.arrClientDataTable.push([
          client.razon_social,
          client.razon_comercial,
          client.nit,
          client.tipo_cliente,
          client.paises.nombre,
        ]);
      });
    });
    // console.log(this.arrClientDataTable);
